...
object CopySimRankExample {
  def main(args: Array[String]): Unit = {
    val delta = 0.8
    val (spark, sc) = MySparkSession.getDefault("SimRankExample")
    //val broadcastnode = sc.broadcast()

    val file = sc.textFile("data/simrank/data1.txt")
    ...
    val graphs = new CopySimRankExample()
    ...
    val result = Skp1.entries.map {
      ...
      .join(indnode.map(x => (x._2, x._1.toLong)))
